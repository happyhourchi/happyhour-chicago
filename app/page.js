...
const SEED = [
  ...
  {id:16,name:"Beatrix River North",address:"519 N Clark St",cuisine:"American",neighborhood:"River North",time:"3-6 PM",days:["Mon","Tue","Wed","Thu","Fri"],deals:["$6 local beers","$7 wines","$8 cocktails","$9 snacks","$12 burgers"],menu:{drinks:["$6 Local Draft Beer","$7 House Wine","$8 Handcrafted Cocktail","$8 Old Fashioned","$9 Seasonal Cocktail"],food:["$9 Snack Plate","$8 Truffle Popcorn","$10 Turkey Meatballs","$12 Mushroom Quinoa Burger","$12 Prime Burger"]},description:"Neighborhood coffeehouse and restaurant with healthy-meets-delicious options and great cocktails. Half-off wine bottles every Monday.",reviews:[],checkins:[],saved:false,isNew:false,addedDate:todayDate},
  ...
  {id:29,name:"Shaw's Crab House",address:"21 E Hubbard St",cuisine:"Seafood",neighborhood:"Streeterville",time:"3-5 PM",days:["Mon","Tue","Wed","Thu","Fri"],deals:["Half-price oysters on the half shell","$10 Bloody Mary","$10 Mignonette Martini","$10 Hugo Spritz","Oyster Bar only"],menu:{drinks:["$10 Shaw's Bloody Mary","$10 Mignonette Martini (Ketel One)","$10 Hugo Spritz","$10 Hot Toddy","$8 House Wine"],food:["Half-price Oysters on Half Shell","Mini Lobster Rolls $15 (3pc)","Happy Hour Tower $57 (oysters+shrimp+lobster)","$8 Clam Chowder","$12 Lobster Bisque"]},description:"Chicago seafood institution since 1984. Half-price oysters Mon-Fri 3-5pm in the Oyster Bar only. Live blues every Thursday 5-8pm.",reviews:[],checkins:[],saved:false,isNew:false,addedDate:todayDate},
...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    try{
      const r=localStorage.getItem('hh_restaurants_v4');
  ...
  function saveR(rs){setRestaurants(rs);try{localStorage.setItem('hh_restaurants_v4',JSON.stringify(rs));}catch(e){}}
  ...
  return(
    <main style={{maxWidth:980,margin:'0 auto',padding:'0 16px 80px',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',background:'#f7f7f8',minHeight:'100vh'}}>
      ...
      {newBadge&&(
        <div style={{position:'fixed',top:20,left:'50%',transform:'translateX(-50%)',background:'#fff',border:'2px solid '+G,padding:'16px 24px',borderRadius:20,zIndex:9999,textAlign:'center',boxShadow:'0 8px 40px rgba(34,197,94,0.25)',minWidth:220}}>
          <div style={{fontSize:40,marginBottom:6}}>{newBadge.icon}</div>
          <div style={{fontSize:10,color:G,fontWeight:700,textTransform:'uppercase',letterSpacing:1,marginBottom:2}}>Badge Unlocked!</div>
          <div style={{fontWeight:800,fontSize:18,color:'#1a1a1a'}}>{newBadge.name}</div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,padding:'20px 0 16px',flexWrap:'wrap'}}>
        <div style={{flex:1}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            <div style={{width:40,height:40,borderRadius:12,background:G,display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,boxShadow:'0 4px 12px rgba(34,197,94,0.3)'}}>🍺</div>
            <div>
              <div style={{fontSize:20,fontWeight:800,color:'#1a1a1a',letterSpacing:-0.5}}>Happy Hour Chicago</div>
              <div style={{fontSize:11,color:'#aaa',marginTop:1}}>{authUser?`Level ${level} · ${profile?.xp||0} XP · ${displayName}`:"Chicago's #1 happy hour finder"}</div>
            </div>
          </div>
        </div>
      ...
      <div style={{display:'flex',background:'#fff',borderRadius:16,padding:4,gap:2,marginBottom:16,border:'1px solid #eee',boxShadow:'0 1px 4px rgba(0,0,0,0.04)'}}>
        {[['list','Browse'],['feed','Activity'],['leaderboard','Top'],['saved','Saved'],['profile','Profile']].map(([t,label])=>(
          <button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:'9px 4px',fontSize:12,border:'none',borderRadius:12,background:tab===t?G:'none',cursor:'pointer',color:tab===t?'#fff':'#999',fontWeight:tab===t?700:400,transition:'all 0.2s',whiteSpace:'nowrap'}}>{label}</button>
        ))}
      </div>
              ...
              {openNowList.length>0?(
                        ...
                        <div key={r.id} onClick={()=>{setDetailTarget(r);setDetailTab('deals');}} style={{minWidth:200,background:'linear-gradient(145deg,#f0fdf4,#dcfce7)',borderRadius:20,padding:'16px 14px',cursor:'pointer',border:'1px solid #bbf7d0',flexShrink:0,position:'relative',overflow:'hidden',boxShadow:'0 2px 12px rgba(34,197,94,0.12)'}}>
                          ...
                          {(r.deals||[]).slice(0,2).map((d,i)=>(
                            <div key={i} style={{fontSize:10,color:'#166534',marginBottom:2,display:'flex',alignItems:'center',gap:3}}>
                              <span style={{fontSize:9}}>🍺</span><span style={{fontWeight:500,lineHeight:1.3}}>{d}</span>
                            </div>
                          ))}
                          <div style={{marginTop:10,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                            {closing?(
                              <span style={{fontSize:9,padding:'2px 6px',borderRadius:99,background:'#fef3c7',color:'#92400e',fontWeight:700}}>⏰ {closing}</span>
                            ):(
                              <span style={{fontSize:9,color:'#15803d',fontWeight:600}}>Until {r.time.split('-')[1]?.trim()}</span>
                            )}
                            {checked
                              ?<span style={{fontSize:9,color:G,fontWeight:800}}>✓ In</span>
                              :<button onClick={e=>{e.stopPropagation();doCheckin(r.id);}} style={{fontSize:9,padding:'4px 8px',borderRadius:99,background:G,color:'#fff',border:'none',cursor:'pointer',fontWeight:800}}>Check in</button>
                            }
                          </div>
                        </div>
                      );
                ...
                <div style={{background:'linear-gradient(135deg,#1a1a1a,#2a2a2a)',borderRadius:20,padding:'20px',marginBottom:4,display:'flex',alignItems:'center',gap:14}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:12,color:'#888',lineHeight:1.4}}>Most run Mon–Fri 3–7pm. Come back then for the best deals!</div>
                  </div>
                </div>
              )}
            </div>
              ...
              return(
                <div key={r.id} onClick={()=>{setDetailTarget(r);setDetailTab('deals');}} style={{background:'#fff',borderRadius:18,padding:'16px',border:'1px solid #eee',cursor:'pointer',position:'relative',overflow:'hidden',boxShadow:'0 1px 4px rgba(0,0,0,0.04)'}}>
                  ...
                  <div style={{display:'flex',gap:4,flexWrap:'wrap',marginBottom:10}}>
                    {active?(
                      <span style={{fontSize:10,padding:'3px 8px',borderRadius:99,background:'#dcfce7',color:'#15803d',fontWeight:700}}>🟢 Open</span>
                    ):(
                      <span style={{fontSize:10,padding:'3px 8px',borderRadius:99,background:'#f5f5f5',color:'#aaa'}}>⏰ {r.time}</span>
                    )}
                  ...
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                    ...
                    <div style={{display:'flex',gap:5}}>
                      {checked
                        ?<span style={{fontSize:10,color:G,fontWeight:700}}>✓ In</span>
                        :<button onClick={e=>{e.stopPropagation();doCheckin(r.id);}} style={{fontSize:10,padding:'4px 10px',borderRadius:99,background:'#dcfce7',color:'#15803d',border:'none',cursor:'pointer',fontWeight:700}}>Check in</button>
                      }
        ...
        <div>
          {!authUser?(
            <div style={{background:'#fff',borderRadius:24,padding:36,textAlign:'center',border:'1px solid #eee'}}>
              <div style={{width:80,height:80,borderRadius:'50%',background:'#dcfce7',display:'flex',alignItems:'center',justifyContent:'center',fontSize:36,margin:'0 auto 16px',boxShadow:'0 4px 20px rgba(34,197,94,0.2)'}}>🍺</div>
              <div style={{fontWeight:800,fontSize:24,marginBottom:8,color:'#1a1a1a'}}>Join the community</div>
              <div style={{fontSize:14,color:'#888',marginBottom:24,lineHeight:1.6,maxWidth:280,margin:'0 auto 24px'}}>Track every happy hour, earn badges, compete on the leaderboard, and save your favorite spots — free forever.</div>
              ...
              <button onClick={()=>setShowAuth(true)} style={{padding:'14px 36px',fontSize:15,borderRadius:99,border:'none',background:G,color:'#fff',cursor:'pointer',fontWeight:800,boxShadow:'0 4px 16px rgba(34,197,94,0.35)'}}>Create free account</button>
            </div>
          ):(
            <div>
              <div style={{background:'#fff',borderRadius:20,padding:20,marginBottom:12,border:'1px solid #eee'}}>
                <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:16}}>
                  <div style={{width:64,height:64,borderRadius:'50%',background:'#dcfce7',color:'#15803d',display:'flex',alignItems:'center',justifyContent:'center',fontSize:24,fontWeight:800,flexShrink:0}}>{initials(displayName)}</div>
                  <div style={{flex:1}}>
                    ...
                    <div style={{display:'flex',gap:6,marginTop:6,flexWrap:'wrap'}}>
                      <span style={{fontSize:11,padding:'3px 10px',borderRadius:99,background:'#dcfce7',color:'#15803d',fontWeight:700}}>Level {level}</span>
              ...
              <div style={{background:'#fff',borderRadius:16,padding:16,marginBottom:12,border:'1px solid #eee'}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(88px,1fr))',gap:8}}>
                  {BADGES.map(b=>{
                    ...
                    return(
                      <div key={b.id} style={{background:earned?'#f0fdf4':'#fafafa',border:'1px solid '+(earned?'#86efac':'#f0f0f0'),borderRadius:14,padding:'10px 6px',textAlign:'center',opacity:earned?1:0.3}}>
                        <div style={{fontSize:24,marginBottom:3}}>{b.icon}</div>
                        <div style={{fontSize:9,fontWeight:700,color:earned?'#15803d':'#aaa',textTransform:'uppercase',letterSpacing:0.3,lineHeight:1.2}}>{b.name}</div>
                      </div>
                    );
                  })}
                </div>
              ...
              <div style={{background:'#fff',borderRadius:16,overflow:'hidden',border:'1px solid #eee',marginBottom:12}}>
                ...
                {dbCheckins.length===0&&<div style={{padding:'20px',fontSize:13,color:'#bbb',textAlign:'center'}}>No check-ins yet — go find a happy hour!</div>}
              ...
              <div style={{background:'#fff',borderRadius:16,overflow:'hidden',border:'1px solid #eee'}}>
                ...
                {[['📍 Check in at a bar','50 XP'],['✍️ Write a review','20 XP'],['➕ Add a restaurant','25 XP'],['⭐ Rate a bar','10 XP']].map(([a,x])=>(
                  <div key={a} style={{display:'flex',justifyContent:'space-between',padding:'10px 16px',borderBottom:'1px solid #f9f9f9',fontSize:13}}>
                    ...
                    <span style={{fontWeight:800,color:G}}>{x}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{background:'#f7f7f8',borderRadius:'24px 24px 0 0',width:'min(640px,100%)',maxHeight:'93vh',overflowY:'auto',paddingBottom:40}}>
              ...
              <div style={{background:active?'linear-gradient(145deg,#f0fdf4,#dcfce7)':'#fff',margin:'12px 16px 0',borderRadius:20,padding:'20px',border:'1px solid '+(active?'#bbf7d0':'#eee')}}>
                ...
                <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                  {checked
                    ?<div style={{fontSize:12,padding:'9px 16px',borderRadius:99,background:'#dcfce7',color:'#15803d',fontWeight:700,display:'flex',alignItems:'center',gap:4}}>✓ Checked in today</div>
                    :<button onClick={()=>doCheckin(r.id)} style={{fontSize:13,padding:'9px 16px',borderRadius:99,background:G,color:'#fff',border:'none',cursor:'pointer',fontWeight:800,boxShadow:'0 4px 12px rgba(34,197,94,0.3)'}}>📍 Check in +50xp</button>
                  }
              ...
              <div style={{display:'flex',gap:6,padding:'12px 16px 0',overflowX:'auto',scrollbarWidth:'none'}}>
                {[['deals','🍺 Deals'],['menu','🍽️ Menu'],['hours','📅 Hours'],['location','📍 Location'],['reviews','⭐ Reviews']].map(([t,label])=>(
                  <button key={t} onClick={()=>setDetailTab(t)} style={{padding:'7px 14px',fontSize:12,borderRadius:99,border:'1px solid '+(detailTab===t?'#1a1a1a':'#e5e5e5'),background:detailTab===t?'#1a1a1a':'#fff',color:detailTab===t?'#fff':'#666',cursor:'pointer',fontWeight:detailTab===t?700:400,whiteSpace:'nowrap',transition:'all 0.15s'}}>
                    {label}
                  </button>
                ))}
              </div>
              ...
              <div style={{padding:'12px 16px 0'}}>
                  ...
                  <div style={{background:'#fff',borderRadius:16,padding:16,border:'1px solid #eee'}}>
                    ...
                    <div style={{display:'flex',gap:5,marginBottom:14}}>
                      {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(d=>{
                        ...
                        return(
                          <div key={d} style={{flex:1,height:42,borderRadius:12,display:'flex',alignItems:'center',justifyContent:'center',fontSize:9,fontWeight:800,background:on?(isToday?G:'#f0fdf4'):'#f5f5f5',color:on?(isToday?'#fff':'#15803d'):'#ccc',border:'1px solid '+(on?(isToday?G:'#dcfce7'):'#eee'),textTransform:'uppercase',letterSpacing:0.3}}>
                            {d.slice(0,2)}
                          </div>
                        );
                      })}
                    </div>
                    <div style={{padding:'12px 14px',background:'#f0fdf4',borderRadius:12,fontSize:14,color:'#15803d',fontWeight:700,display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                      <span>⏰</span><span>{r.time}</span>
                    </div>
                    <div style={{padding:'10px 14px',background:'#f8f8f8',borderRadius:10,fontSize:12,color:'#888'}}>
                      Today is <strong style={{color:'#555'}}>{today}</strong> — {active?<span style={{color:G,fontWeight:700}}>happy hour is happening now! 🎉</span>:<span>happy hour is not currently active.</span>}
                    </div>
                    <div style={{marginTop:14}}>
                      <div style={{fontSize:12,color:'#bbb',marginBottom:8,fontWeight:600}}>Rate this place</div>
                      <div style={{display:'flex',gap:6}}>
                        {[1,2,3,4,5].map(star=>(
                          <span key={star} onClick={()=>doRate(r.id,star)} style={{cursor:'pointer',fontSize:30,color:ur>=star?'#f59e0b':'#e8e8e8',lineHeight:1}}>★</span>
                        ))}
                ...
                {detailTab==='location'&&(
                  <div style={{background:'#fff',borderRadius:16,padding:16,border:'1px solid #eee'}}>
                    ...
                    <div style={{fontSize:13,color:'#888',marginBottom:4}}>{r.address}</div>
                    <div style={{fontSize:13,color:'#888',marginBottom:16}}>Chicago, IL</div>
                    <div style={{fontSize:12,color:'#aaa',marginBottom:14,padding:'10px 12px',background:'#f8f8f8',borderRadius:10,display:'flex',alignItems:'center',gap:6}}>
                      <span>📍</span><span>{r.neighborhood} neighborhood</span>
                    </div>
                    <a href={mapsUrl} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,fontSize:14,padding:'14px',borderRadius:14,background:'#1d4ed8',color:'#fff',textDecoration:'none',fontWeight:700,boxShadow:'0 4px 12px rgba(29,78,216,0.25)'}}>
                    ...
                    {(r.reviews||[]).map(rv=>(
                      <div key={rv.id} style={{background:'#f9f9f9',borderRadius:14,padding:'12px',marginBottom:8}}>
                        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}>
                          ...
                          <div style={{flex:1}}>
                            <div style={{fontSize:13,fontWeight:700,color:'#1a1a1a'}}>{rv.author}</div>
        ...
        <div onClick={e=>{if(e.target===e.currentTarget){setShowReview(false);setReviewForm({stars:5,text:''});}}} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1001,padding:16}}>
          <div style={{background:'#fff',borderRadius:24,padding:28,width:'min(460px,100%)'}}>
            ...
            <div style={{display:'flex',gap:8,marginBottom:20,justifyContent:'center'}}>
              {[1,2,3,4,5].map(star=><span key={star} onClick={()=>setReviewForm(f=>({...f,stars:star}))} style={{fontSize:44,cursor:'pointer',color:reviewForm.stars>=star?'#f59e0b':'#e8e8e8',lineHeight:1}}>★</span>)}
            </div>
            ...
            {aiResults.map((r,i)=>(
              <div key={i} onClick={()=>addFromAI(r)} style={{background:'#f9f9f9',borderRadius:14,padding:'14px',marginBottom:8,cursor:'pointer',border:'1px solid #eee'}}>
                ...
                <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
                  {(r.deals||[]).slice(0,3).map((d,i)=><span key={i} style={{fontSize:10,padding:'2px 8px',borderRadius:99,background:'#f0fdf4',color:'#15803d',fontWeight:600}}>{d}</span>)}
                </div>
        ...
        <div onClick={e=>{if(e.target===e.currentTarget){setShowAuth(false);setAuthError('');}}} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1001,padding:16}}>
          <div style={{background:'#fff',borderRadius:24,padding:28,width:'min(400px,100%)'}}>
            <div style={{textAlign:'center',marginBottom:24}}>
              <div style={{width:64,height:64,borderRadius:'50%',background:'#dcfce7',display:'flex',alignItems:'center',justifyContent:'center',fontSize:30,margin:'0 auto 14px',boxShadow:'0 4px 20px rgba(34,197,94,0.2)'}}>🍺</div>
            ...
            <button onClick={authMode==='login'?signInWithEmail:signUpWithEmail} disabled={authLoading} style={{width:'100%',padding:'13px',fontSize:14,borderRadius:14,border:'none',background:G,color:'#fff',cursor:'pointer',fontWeight:800,marginBottom:14,boxShadow:'0 4px 16px rgba(34,197,94,0.3)'}}>
